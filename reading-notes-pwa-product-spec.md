...
- 卡片仅展示必要信息，避免全文铺开。
- 首页和笔记库采用相同层级：有原文时原文最先、保留换行并最多 5 行；下方“我的思考”次级字号/颜色、最多 2 行。仅有感悟则作为主体，不显示空原文。来源、最多 3 个标签、时间为辅助信息；追加内容仅计数。详情/复习保留完整原文、原始感悟及追加时间线。
...
11. 临时 Session 请求失败保留已确认的 Auth，提供重试入口；后续 Session 或认证同步成功清除旧错误，迟到旧请求不得恢复过期状态。同步错误与冲突分开呈现。
12. 设置提供冲突对照与处理页：展示双方完整快照、版本、墓碑和副本入口，区分内容差异/仅版本时间差异。用户核对后可保留双方并仅标记本机 `resolvedAt`，不删除审计记录或任何业务副本，不改同步协议。
13. 云同步不等于端到端加密；涉及敏感生活感悟时必须在产品中如实说明。

## 13. 视觉与交互方向

- 气质：克制、安静、像纸张但不做拟物书架。
- 浅色：纸白背景、深灰正文、低饱和墨绿色作为主色；清晰系统字阶、精细圆角和安静的留白，不复制系统应用。
- 深色：非纯黑背景，保持原文与感悟的层级差异。
- 使用系统中文字体栈，避免首屏依赖网络字体。
- 原文适合稍大行距；感悟用不同背景或左边线区分，但保持高对比度。
- 动效只用于切换、保存反馈和复习卡片，不使用大面积花哨过渡。
- 原创图标以书页、摘录、回想箭头组成；维护 SVG 源并生成 PWA 192/512/maskable 512 和 iPhone touch 180 图标，maskable 主体位于安全区。实机主屏幕检查由产品所有者完成，不以模拟截图宣称 iPhone 验收通过。
